...
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games ON teams.team_id = games.opponent_id OR teams.team_id = games.winner_id WHERE year = 2014 AND round = 'Eighth-Final' GROUP BY name ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT ON (year) year, teams.name FROM games JOIN teams ON games.winner_id = teams.team_id WHERE games.round = 'Final' ORDER BY year, teams.name")"
